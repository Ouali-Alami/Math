def IsDivisbleByThree(x) -> bool:
    ...
    # for this alogrithm to be useful for something you must imperatively convert the number into a string in order 
    # to obtain each digit otherwise it is strictly useless and it is even worse because you will apply the modulo on each of the digits of the number
    number = str(x)
    sum = 0
    for digit in number:
        sum += int(digit)
